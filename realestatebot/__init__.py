...
def distance_from_target(
	...
	if not target_lat or not target_lon:
		...
		return None
	d = (degrees_to_km * ((lat - target_lat)**2 + (lon - target_lon)**2)**0.5) if lat and lon else None
...
def main(
	...
	with alive_bar(
		...
		for listing in raw_listings:
			...
			if all([
					(price_range[0] <= price <= price_range[1]) or (not price_range[0] and not price_range[1]),
