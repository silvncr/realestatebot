...
target_lat_env = float(eval(f'{getenv('target_lat', 0)}')) or None
target_lon_env = float(eval(f'{getenv('target_lon', 0)}')) or None
price_low_env = float(eval(f'{getenv('price_low', 0)}')) or 10_000
price_high_env = float(eval(f'{getenv('price_high', 0)}')) or 10_000_000
			...
			for result in tier['results']:
				...
				for listing in [result]:
					for f in [
						...
						find_keys(x, 'price'),
							lambda x:
						find_keys(x, 'priceRange'),
							lambda x:
						find_keys(x, 'pricerange'),
							lambda x:
						find_keys(x, 'priceText'),
							lambda x:
						find_keys(x, 'pricetext'),
					]:
						if f(listing):
...
def distance_from_target(
	...
	if not target_lat or not target_lon:
		return None
	return 111.32 * ((lat - target_lat)**2 + (lon - target_lon)**2)**0.5 if lat and lon else None
...
def main(
	...
	if is_offline:
		...
		# load data from out_raw.json
		print('\n\tOffline mode: loading from \'out_raw.json\'..')
		with open(os_path.join(sys_path[0], 'out_raw.json'), 'tr') as file:
	...
	# online mode
		...
		extra_raw_listings = []
		with alive_bar(
				len(postcodes)*len(states),
				title='Searching..',
				title_length=12,
		) as bar:
			for postcode, state in product(postcodes, states):
				bar.text(f'| Current: ({state}, {postcode})')
...
if __name__ == '__main__':
	...
	# save search results
	if is_debug:
		with open(os_path.join(sys_path[0], 'out_raw.json'), 'w', encoding='utf-8') as file:
			dump(raw, file, indent=4)

	# save listings
	with open(os_path.join(sys_path[0], 'out.json'), 'w', encoding='utf-8') as file:
		dump(processed_listings, file, indent=4)
	with open(os_path.join(sys_path[0], 'out.csv'), 'w', encoding='utf-8') as file:
		data.to_csv(file, index=False)

	# remove double newlines from csv
